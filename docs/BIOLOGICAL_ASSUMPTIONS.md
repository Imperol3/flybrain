...
## 4d. Embodied motor decoder and closed-loop window — engineering choices

Viewer V2 preserves the real LIF network state between short windows, but
the translation from descending-neuron activity to body movement is
necessarily an engineered interface: FAFB contains the brain connectome,
not a complete ventral nerve cord, muscles, wings, legs, or biomechanics.

| decision | value used | interpretation |
|---|---:|---|
| Interactive simulation window | 25 ms | Balance between closed-loop responsiveness and whole-connectome runtime; state persists across windows |
| DNa02 turn gain | 0.012 rad/s per Hz differential | Converts `DNa02_L - DNa02_R` mean rate to yaw velocity |
| Maximum yaw velocity | 2.4 rad/s | Prevents an unbounded visual/body rotation |
| Orienting speed | 1.5 cm/s | Slow movement while steering is present but escape is not latched |
| Escape speed | 12.0 cm/s | Demonstration body speed after DNp01 crosses the existing escape threshold |
| Takeoff height | 2.2 cm | Visual flight arc; not a measured Drosophila jump-height claim |
| Maximum interactive run | 3,000 ms | Product/runtime guardrail, not a biological constant |

The stimulus is recomputed from the body's updated position and heading
before every window. Spatial routing remains the coarse left/right
hemifield rule from section 4c; Viewer V2 does not add a retinotopic eye
model. These values live in `simulation/closed_loop.py` as
`MotorDecoderConfig` so they remain visible, testable, and replaceable.
